...
from cloudy.exploration.lightning_limits import MAX_RAW_LIGHTNING_CHART_EVENTS
...
# The point budget: never ship more series points than a chart that wide can
# actually resolve. We aim for ~1.5 points per pixel, floored so a tiny viewport
# still gets a usable series and capped so a giant one can't ask for an unbounded
# payload. The MAX_RAW_* / SCAN limits are the hard backstops that turn an
# abusive request into a typed 413 instead of a melted backend.
...
# Rejection is a first-class result, not a 500. It always carries a
# `suggested_aggregation` — the resolution that *would* fit the budget — so the
# client can retry coarser instead of being told a bare "too big".
@dataclass(frozen=True)
...
def choose_resolution(
    ...
    """Pick the finest resolution that still fits the point budget.

    Raw (unbucketed) is the ideal when it's small enough to be honest: for cloud
    that means a short window whose hourly points fit the budget; for lightning it
    means a single day whose actual event count fits. Otherwise we walk from fine
    to coarse and take the first bucket size under budget, falling back to yearly
    when even months overflow.
    """
...
# The fine end of the ladder is gated by span: sub-daily buckets only make sense
# for short windows, so a multi-year request never even considers hourly. Cloud
# and lightning currently share the same ladder; they're spelled separately so
# either can diverge (e.g. lightning gaining a finer grain) without disturbing
# the other.
...
def _reject_if_unsafe(
    ...
    # Three ways a request can be unsafe: a raw response with too many points, an
    # aggregation that still overflows the budget (usually a hand-picked grain too
    # fine for the span), or a lightning scan that would touch too many rows even
    # though the *output* is small. Each rejection suggests the auto resolution as
    # the safe retry.
