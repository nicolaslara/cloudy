"""Typed response shapes for GET /api/v1/exploration/cloud."""
...
class CloudPeriod(TypedDict):
    ...
    # The honesty triple: observed vs expected (and the gap) so a bucket can't
    # quietly imply full coverage when stations were missing. For the Sweden
    # aggregate, expected counts every station that participated in the window —
    # see cloud_series._SWEDEN_ROLLUP_SQL.
    observed_count: int
    expected_count: int
    missing_count: int
