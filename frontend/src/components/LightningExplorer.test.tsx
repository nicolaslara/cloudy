...
test("requests series format with Sweden scope when no location is selected", async () => {
  ...
  const lightningUrl = String(
    fetchMock.mock.calls.find(([input]) => String(input).includes("/api/v1/lightning?"))?.[0],
  );
  ...
  const cloudUrl = String(
    fetchMock.mock.calls.find(([input]) => String(input).includes("/api/v1/cloud"))?.[0],
  );
  expect(cloudUrl).toContain("/api/v1/cloud");
...
test("chart controls expose only safe aggregations", async () => {
  ...
  const aggregations = fetchMock.mock.calls
    ...
    .filter((url) => url.pathname === "/api/v1/lightning")
  ...
  await waitFor(() => {
    expect(
      fetchMock.mock.calls.some(
        ([input]) =>
          String(input).includes("/api/v1/cloud") &&
