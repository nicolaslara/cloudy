...
import { countAxisMax, logCountAxisMax } from "../../../lib/chartAxis";
import {
  ALL_BOTTOM,
  ALL_TOP,
  AXIS_LABEL,
 ...
 * `filterMode: "none"` is deliberate — zooming must not drop out-of-window data
 * points, since useECharts reads the visible range back off this dataZoom and a
 * filtered axis would lie about what's actually loaded.
 */
...
export function toChartOption(
  ...
  const dischargeSeries: BarSeriesOption[] = [
    ...
    },
    {
      ...
      // CG is a subset of all discharges, so we draw it on top of the All bars
      // (barGap -100% + higher z) rather than beside them — same baseline, the
      // shorter CG bar reads as a fill inside the taller All bar.
      barGap: "-100%",
      z: 3,
    },
  ];
  ...
  return {
    ...
    yAxis: {
      ...
      name: "Strikes",
      min: 0,
      // `max` is the axis-headroom *function* (not a number); ECharts calls it
      // with the data extent so the cap tracks each window's tallest bar.
