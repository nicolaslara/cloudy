...
/**
 * Decode the API's packed stroke rows — positional tuples, not objects, to keep
 * the per-stroke payload small over the wire. Order is [lon, lat, peakKa, cg,
 * ts]; `cg` is 0/1, `ts` is epoch seconds. `day` is the UTC calendar day (our
 * canonical bucket key); it is *not* localized to Stockholm.
 */
