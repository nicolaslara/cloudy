...
# deliberately leave `source` out and keep the project build-config-only: deploys
# happen via `wrangler pages deploy frontend/dist` (or a CI step) against this
# project. Simpler, host-agnostic, and nothing here needs a real OAuth link to
# plan. Add a `source` block later if git-connected builds are wanted.
